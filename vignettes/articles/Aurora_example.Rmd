...
```{r, eval=FALSE}
clean.controls(flow.control, asp)
```
...
![Spectral Trace](figures/Initial Universal Negative autospectral spectra.jpg)

Now we can unmix.
By default, AutoSpectral extracts a single AF parameter. To unmix without AF 
extraction, remove this from the spectral matrix:
```{r, eval=FALSE}
