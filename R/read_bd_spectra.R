...
read.bd.spectra <- function( fcs.file ) {
  ...
  spill.vector <- suppressWarnings( read.FCSheader( fcs.file, keyword = "SPILL" )[[ 1 ]] )
  spill.vector <- strsplit( spill.vector, "," )[[ 1 ]]
  n.detectors <- as.numeric( spill.vector[ 1 ] )
  detector.names <- spill.vector[ 2:( n.detectors + 1 ) ]
  
  spill.matrix <- matrix( as.numeric( spill.vector[ ( n.detectors + 2 ) : length( spill.vector ) ] ),
                          nrow = n.detectors, ncol = n.detectors )
  spill.matrix <- t( spill.matrix )
  
  marker.names <- suppressWarnings( read.FCSheader( fcs.file, 
                                                    keyword = "BDSPECTRAL UNMIXED" )[[ 1 ]] )
